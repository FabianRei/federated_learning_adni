...
from dl.neural_network.train_test import train, test
...
log_path = os.path.join(out_path, f"training_log_{time_stamp}.txt")
sys.stdout = Logger(log_path)
csv_path = os.path.join(out_path, f"train_test_accuracy_{time_stamp}.csv")
...
for i in range(num_epochs):
    ...
    if save_pred_labels:
        pickle_fn = os.path.join(out_path, f"epoch_{i}_pred_labels_train_test_epoch_{time_stamp}")
